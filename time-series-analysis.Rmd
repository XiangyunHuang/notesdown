...
```{r sunspot, fig.showtext=TRUE, fig.cap="月均太阳黑子数"}
...
```{r other-ts, fig.cap="时间序列：非平稳、周期性、非线性", fig.subcap=c("1960-1980年强生公司每股季度收益", "1949-1960年月均航班乘客数量", "1920-1939 年诺丁汉月均气温", "1821-1934 年加拿大山猫陷阱数量"), fig.ncol=2,fig.width=8,fig.height=6,fig.show='hold'}
plot(JohnsonJohnson)
plot(AirPassengers)
plot(nottem)
plot(lynx)
...
```{r hk-stock-data,cache=is.na(Sys.getenv('CI', NA)),eval=!is.na(Sys.getenv('CI', NA))}
# 美团
...
```
```{r, echo=knitr::is_html_output(), eval=knitr::is_html_output() & !is.na(Sys.getenv('CI', NA)), fig.cap="港股走势"}
...
```{r us-stock-data,cache=is.na(Sys.getenv('CI', NA)),eval=!is.na(Sys.getenv('CI', NA))}
# 拼多多
...
```
```{r, echo=knitr::is_html_output(), eval=knitr::is_html_output() & !is.na(Sys.getenv('CI', NA)), fig.cap="美股走势"}
highchart(type = "stock") %>% 
...
```{r coe-data,cache=TRUE,eval=FALSE}
coe <- quantmod::getSymbols("COE", auto.assign = FALSE, src = "yahoo", from = '2016-06-30')
```

读者可以从雅虎财经获取数据源 <https://finance.yahoo.com/>

```{r coe-close-price, eval=knitr::is_latex_output() & !is.na(Sys.getenv('CI', NA)), fig.cap="51Talk公司上市以来的股价走势", echo=knitr::is_latex_output()}
plot(coe[, "COE.Close"],
...
```{r coe-price, eval=!is.na(Sys.getenv('CI', NA)), fig.cap="CEO 股价变化趋势", fig.asp=1, out.width="85%"}
autoplot(coe)
```
