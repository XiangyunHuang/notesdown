...
```{r,fig.cap="rect 函数画长方形",title.mar=TRUE,fig.asp=1}
# From the manual
ch.col <- c(
...
```{r,title.mar=TRUE,fig.cap="两个 Y 轴",fig.asp=1,fig.width=6.5,out.width="65%"}
# 双Y轴
N <- 200
x <- seq(-4, 4, length = N)
y1 <- sin(x)
...
```{r gap-axis,fig.asp=1,fig.cap="gap.axis用法",title.mar=TRUE,fig.width=5.5,out.width="55%"}
...
```{r,fig.cap="标题含有数学公式",title.mar=TRUE,fig.asp=1,out.width="45%",fig.width=4.5}
...
```{r,fig.cap="批量生成函数图形",title.mar=TRUE,fig.asp=1,out.width="45%",fig.width=4.5}
x <- seq(-5, 5, length = 200)
for (i in 1:4) { # 画四个图
  y <- sqrt(i + x^2)
  plot(y ~ x,
...
```{r,fig.cap="三角函数添加图例",title.mar=TRUE,fig.asp=0.8}
...
```{r,fig.cap="设置图例的位置",title.mar=TRUE,fig.asp=0.8}
xmin <- par("usr")[1]
xmax <- par("usr")[2]
ymin <- par("usr")[3]
ymax <- par("usr")[4]
...
```{r,fig.cap="insert 函数微调图例位置",title.mar=TRUE,fig.asp=0.8}
plot(y ~ x,
  type = "l", lwd = 3,
  ylab = "", xlab = "angle", main = "Trigonometric functions"
)
...
```{r,fig.cap="将图例放在绘图区域外面",title.mar=TRUE,fig.asp=0.8}
...
```{r,title.mar=TRUE,fig.asp=1,out.width="65%",fig.width=6.5}
...
```{r,title.mar=TRUE,fig.asp=1,out.width="55%",fig.width=5.5}
...
```{r,title.mar=TRUE,fig.asp=1,out.width="75%",fig.width=7.5}
library(survival)
surv.days <- Surv(days)
...
```{r,fig.cap="矩阵图",fig.width=4, fig.height=4, out.width="75%"}
# 自带 layout
...
```{r lattice-settings, fig.width=5, fig.height=4, out.width="75%"}
show.settings()
```
...
```{r map,fig.width=4,fig.height=6,out.width="75%"}
library(maps)
library(mapproj)
