...
remote_pkgs <- c(equatiomatic = "datalorax", cmdstanr = "stan-dev", hadley = "emo")
...
```{r god-play-games,fig.cap="上帝在掷骰子吗？",dev='tikz',fig.process=to_png,cache=TRUE,fig.width=4,fig.height=4,fig.asp=1,echo=FALSE}
# 随机，先验分布
set.seed(2018)
...
```{r stats-level,fig.cap="(ref:stats-level)",fig.scap="(ref:stats-level-s)",echo=FALSE,fig.width=5,fig.height=5}
par(mar = rep(0.5, 4))
...
第 \@ref(chap:data-transportation) 章介绍数据导入导出 data.table 用于 csv 文件，而 openxlsx 用于 xlsx 文件，第 \@ref(chap:data-manipulation) 章介绍数据操作 Base R 和 data.table，第 \@ref(chap:data-visualization) 章介绍数据可视化 ggplot2

## 致谢 {#sec:acknowledgments .unnumbered}
