...
x = seq(from = 4, to = 20, length.out = 300)
y = dnorm(x, mean = 10, sd = 1)
yy = dnorm(x, mean = 12, sd = 2)
library(magrittr)
...
plotly::plot_ly(
  ...
  hoverinfo = "text",
  name = plotly::TeX("\\mathcal{N}(10,1^2)"),
  ...
  plotly::add_trace(
    ...
    hoverinfo = "text",
    name = plotly::TeX("\\mathcal{N}(12, 2^2)"),
  ...
  # plotly::add_annotations(
  #   x = 4, y = 1 / sqrt(2 * pi), font = list(size = 100), showarrow = F,
  #   text = plotly::TeX("\\frac{1}{\\sqrt{2\\pi}}")
  # ) %>%
  # plotly::add_annotations(
  #   x = 4, y = 1 / (2 * sqrt(2 * pi)), font = list(size = 100), showarrow = F,
  #   text = plotly::TeX("\\frac{1}{2\\sqrt{2\\pi}}")
  # ) %>%
  plotly::layout(
    shapes = list(
      # hline(y = 1 / sqrt(2 * pi), color = "#F27B0C"),
      # hline(y = 1 / (2 * sqrt(2 * pi)), color = "#F27B0C"),
      vline(x = 10, color = "#F27B0C"),
      vline(x = 12, color = "#F27B0C")
    ),
...
```{r god-play-games,fig.width=5,fig.height=5,fig.cap="上帝在掷骰子吗？",dev='tikz',fig.process=to_png,cache=TRUE,echo=FALSE}
# 随机，先验分布
# 正态分布是服从一定的微分方程
set.seed(2018)
