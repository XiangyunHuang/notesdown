...
dat <- subset(x = dat, subset = !grepl("Xiangyun", x = author))
dat <- subset(x = dat, subset = !grepl("湘云", x = author))
dat <- aggregate(data = dat, commit ~ author, FUN = length)
dat <- transform(dat, author = gsub(pattern = "<.*?>", replacement = "", x = author))
knitr::kable(dat[order(-dat$commit), ],
