...
x = seq(from = -3, to = 9, length.out = 301)
y = dnorm(x, mean = 1, sd = 1)
yy = dnorm(x, mean = 2, sd = 2)
library(magrittr)
...
plotly::plot_ly(
  ...
  mode = "lines", name = plotly::TeX("\\mathcal{N}(1,1)"),
  line = list(shape = "spline", color = "#5CB85C")
) %>%
  plotly::add_trace(
    x = x, y = yy, type = "scatter",
    fill = "tozeroy",
    text = ~ paste0(
      "x：", x, "<br>",
      "f(x)：", round(yy, 3), "<br>"
    ...
    mode = "lines", name = plotly::TeX("\\mathcal{N}(2, 2^2)"),
