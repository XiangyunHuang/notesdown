...
library(magrittr)
git_log(max = 1000) %>%
  subset(subset = !grepl("Xiangyun Huang", x = author)) %>% # 过滤
  aggregate(data = ., formula = commit ~ author, FUN = length) %>% # 分组计数
  .[order(-.$commit), ] %>% # 排序
  transform(author = gsub(pattern = "<.*?>", replacement = "", x = author)) %>% # 变换
  knitr::kable(
