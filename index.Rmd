...
geometry:
  ...
  - lmargin=3.0cm
  - rmargin=2.0cm
classoption: "UTF8,twoside,openany,fontset=adobe,zihao=false,table"
bibliography: 
  - book.bib
  - packages.bib
link-citations: yes
...
remote_pkgs <- c(hadley = "emo")

lapply(c(
  "bookdown", "desc", "remotes", "reticulate", "rootSolve", "gert", "Kendall", "SuppDists", "pspearman"
), function(pkg) {
...
```{r normal-dist,fig.cap="统计学的世界",eval=knitr::is_html_output(), echo=knitr::is_html_output()}
...
```{r confidence-belt,opts.label='fig.small',fig.cap="(ref:confidence-belt)",fig.scap="(ref:confidence-belt-s)",echo=FALSE,fig.pos = "!htb"}
...
二项分布 $B(n,\theta)$ 成功概率 $\theta$，固定样本量 $n = 10$，分不同的分位点 $q = 2, 4, 6$ 绘制概率随成功概率 $\theta$ 的变化

```{r pbinom,fig.cap="概率分布",echo=FALSE,opts.label='fig.small'}
library(ggplot2)
...
```{r god-play-games,opts.label='fig.small',fig.cap="上帝在掷骰子吗？",dev='tikz',fig.process=to_png,cache=TRUE,echo=FALSE}
# 随机，先验分布
# 正态分布是服从一定的微分方程
set.seed(2018)
...
```{r faithful, fig.cap="散点图：faithful 数据集",opts.label='fig.small',echo=FALSE}
plot(faithful,
...
```{r ab-test,fig.asp="A/B 测试：功效",opts.label='fig.small',echo=FALSE}
