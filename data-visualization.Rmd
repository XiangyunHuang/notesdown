...
```{r calendar-airquality,fig.cap="1973年纽约5月至9月的气温变化",fig.width=8,fig.height=4}
airquality %>%
  transform(Year = 1973) %>%
  transform(Date = seq.Date(from = as.Date("1973-05-01"), to = as.Date("1973-09-30"), by = "day")) %>%
  transform(Week = as.integer(format(Date, "%W"))  ) %>%
  transform(Weekdays = factor(weekdays(Date, abbreviate = T), levels = c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"))) %>%
  ggplot(aes(x = Week, y = Weekdays, fill = Temp)) +
  scale_fill_binned(name = "Temp (F)", type = "viridis", limits = c(0, max(airquality$Temp))) +
  geom_tile(color = "white", size = 0.4) +
...
hcmap(
  ...
  hc_colorAxis(
    stops = color_stops(colors = hcl.colors(n = 10, palette = "inferno")),
    type = "logarithmic"
  )
```
...
[**GGally**](https://github.com/ggobi/ggally)、 [**ggupset**](https://github.com/const-ae/ggupset) 和 [**ggpubr**](https://github.com/kassambara/ggpubr) 高度定制了一些组合统计图形，以 ggpubr 为例，见图 \@ref(fig:ggpubr-composite)。
