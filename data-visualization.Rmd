...
```{r system-fonts-liberation, fig.width=6.5, fig.height=6, fig.cap="调用系统字体绘图", out.width="75%"}
library(extrafont)
...
```{r font-in-ggplot, fig.width=4, fig.height=4, fig.showtext=TRUE, fig.cap="在 ggplot2 绘图系统中设置中英文字体"}
...
```{r hrbrthemes, fig.cap="调用 hrbrthemes 包设置字体主题", fig.width=4, fig.height=4, out.width="75%"}
library(hrbrthemes)
...
```{r arial-narrow, fig.cap="默认字体 Arial Narrow", out.width="75%", fig.width=4, fig.height=4}
ggplot(mtcars, aes(mpg, wt)) +
  geom_point() +
  labs(
    x = "Fuel efficiency (mpg)", y = "Weight (tons)",
...
```{r xkcd-graph, fig.cap = "漫画风格的字体方案", dev = if (knitr::is_html_output()) "svg" else if (knitr::is_latex_output()) "cairo_pdf" else "png", out.width="75%", fig.width=4, fig.height=4}
library(extrafont)
library(xkcd)
...
data.frame(
  ...
  theme_void()
```
```{r emoji-fonts-online, echo=!is.na(Sys.getenv('CI', NA)), eval=!is.na(Sys.getenv('CI', NA)), fig.cap="表情字体", out.width="75%", fig.width=4, fig.height=4}
data.frame(
