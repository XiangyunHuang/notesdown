...
```bash
# 等宽字体
sudo dnf install -y google-noto-mono-fonts google-noto-sans-fonts google-noto-serif-fonts
```

这四款字体分别用来替换 Windows 系统上的 Courier New/Arial Narrow/Arial/Times New Roman 字体

```bash
sudo dnf install -y liberation-mono-fonts liberation-narrow-fonts liberation-sans-fonts liberation-serif-fonts 
```
...
```{r pressure,fig.asp=.7,fig.cap="调用系统字体绘图"}
plot(data = pressure, pressure ~ temperature, 
     xlab = "Temperature (deg C)", ylab = "Pressure (mm of Hg)",
     col.lab = "red", col.axis = "blue", font.lab = 3, font.axis = 2, family = "DejaVu Sans")
title(main = "Vapor Pressure of Mercury as a Function of Temperature", family = "DejaVu Serif", font.main = 3)
title(sub = "Data Source: Weast, R. C", family = "DejaVu Mono", font.sub = 1)
```
...
```bash
sudo dnf install -y google-noto-emoji-color-fonts google-noto-emoji-fonts
```
