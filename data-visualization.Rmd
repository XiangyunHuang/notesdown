...
-   Thomas Lin Pedersen [Drawing Anything with ggplot2](https://github.com/thomasp85/ggplot2_workshop)
...
```{r state-crimes,fig.cap="1975年美国各州犯罪事件",fig.width=5,fig.height=4}
library(maps)
...
```{r incorrect-map,fig.cap="中国及其周边",fig.width=5,fig.height=4}
library(maps)
library(mapdata)
...
```{r normal-violin,fig.cap="正态分布的概率密度曲线"}
...
p +
  ...
  labs(y = "Second Principal Components", x = "First Principal Components")
...
```{r pca-ggplot2,fig.cap="主成分分析",fig.width=4,fig.height=4}
library(ggfortify)
...
[Tidy Tuesday Vacination](http://paulnice.datasorcery.tech/posts/tidy-tuesday-vacination/)

[GGally](https://github.com/ggobi/ggally)、 [ggupset](https://github.com/const-ae/ggupset) 和 [ggpubr](https://github.com/kassambara/ggpubr) 高度定制了一些统计图形，以 ggupset 为例
...
[mapmate](https://github.com/leonawicz/mapmate) 制作地图方面的三维可视化图形和 [gganimate 案例合集](https://github.com/ropenscilabs/learngganimate) 都是不错的入门材料。此外还有 rgl 可以制作真三维动态图形，支持缩放、拖拽、旋转等操作，magick 可以将一系列静态图形合成动态图形。
...
cup_curve <- function(n = 100, theta = 3, phi = 30, cup = "A") {
  data.frame(x = seq(0.00001, 1, length.out = n), cup = cup) %>%
    transform(y = theta * x * log(x, base = 10) - 1 / phi * exp(-(phi * x - phi / exp(1))^4))
}

mapply(
  FUN = cup_curve, theta = c(A = 3, B = 2.8, C = 2.5, D = 2.2, E = 2),
...
另外值得一提的是 [hrbrthemes](https://github.com/hrbrmstr/hrbrthemes) 包，除了定制了很多 ggplot2 主题，它还打包了很多的字体主题。比如默认主题 `theme_ipsum()` 使用 Arial Narrow 字体，如果没有该字体就自动寻找系统中的替代品，如图 \@ref(fig:hrbrthemes) 实际使用的是 Nimbus Sans Narrow 字体，因为在 GitHub Action 中，我实际使用的测试环境是 Ubuntu 18.04，该系统自带 Nimbus Sans Narrow 字体，Arial Narrow 毕竟是 Windows 上的闭源字体。
...
ggplot(mtcars, aes(mpg, wt)) +
  ...
  # theme_ipsum() 
  theme_ipsum(base_family = "Nimbus Sans Narrow")
```
...
```{r liberation-sans-narrow,fig.cap="用 Liberation Sans Narrow 字体替换默认字体 Arial Narrow"}
ggplot(mtcars, aes(mpg, wt)) +
  geom_point() +
  labs(
    x = "Fuel efficiency (mpg)", y = "Weight (tons)",
...
Winston Chang 将 Paul Murrell 的 Computer Modern 字体文件打包成 [fontcm](https://github.com/wch/fontcm) 包 [@fontcm]，fontcm 包可以在 Base R 图形中嵌入数学字体 [^2]，图形中嵌入重音字符 [^3]。下面先下载、安装、加载字体，

[^2]: <https://www.stat.auckland.ac.nz/~paul/R/CM/CMR.html>

[^3]: <https://www.stat.auckland.ac.nz/~paul/Reports/maori/maori.html>
...
阐述色理 HCL RGB CMYK Adobe RGB sRGB P3广色域\
2K 4K 5K 视网膜屏 普清 高清 超高清 全高清 显示器 打印机 照相机

为什么需要这么多颜色模式

知道颜色模式及其之间的转化 颜色空间及其区别 HCL and HSV color spaces 基于网页、人眼、打印机使用不同的颜色模式为何
