...
library(tufte) 
library(tint) # Warning message: Function 'sans_serif' not applicable for package 'tint'. It may be removed in a future release. 
```

# Before We Get Started...

```{r fig-nocap-margin-first, fig.margin=TRUE, fig.width=4, fig.height=4, cache=FALSE, echo=FALSE}
library(ggplot2)
...
```{r fig-two-together, fig.cap="Two plots in one figure environment.", fig.show='hold', cache=FALSE}
...
```{r fig-two-separate, ref.label='fig-two-together', fig.cap=sprintf("Two plots in separate figure environments (the %s plot).", c("first", "second")), cache=FALSE}
```
...
```{r fig-margin-together, fig.margin=TRUE, fig.show='hold', fig.cap="Two plots in one figure environment in the margin.", fig.width=3.5, fig.height=2.5, cache=FALSE}
p
...
```{r fig-margin-separate, fig.margin=TRUE, fig.cap=sprintf("Two plots in separate figure environments in the margin (the %s plot).", c("first", "second")), fig.width=3.5, fig.height=2.5, cache=FALSE}
...
```{r fig-nocap-margin, fig.margin=TRUE, fig.width=3.5, fig.height=2, cache=FALSE}
# a boxplot of weight vs transmission; this figure
# will be placed in the margin
ggplot(mtcars2, aes(am, wt)) + geom_boxplot() +
  coord_flip()
```
```{r fig-nocap-main, cache=FALSE}
# a figure in the main column
p <- ggplot(mtcars, aes(wt, hp)) + geom_point()
p
```
```{r fig-nocap-fullwidth, fig.fullwidth=TRUE, fig.width=10, fig.height=3, cache=FALSE}
# a fullwidth figure
