...
<!-- [shinybrms](https://github.com/fweber144/shinybrms) 贝叶斯模型求解器，基于 shiny，Stan 写模型，模型库，编译好供大家调用，用 shiny 包装好，brms/rstan 的后端为 cmdstanr，代码版本管理，模型库管理，软件环境管理 -->
...
[rstan](https://github.com/stan-dev/rstan) [brms](https://github.com/paul-buerkner/brms)  [shinybrms](https://github.com/fweber144/shinybrms) [rethinking](https://github.com/rmcelreath/rethinking)
...
```{stan output.var="normal_gp",eval=FALSE}
data {
  int<lower=1> N;
  real x[N];
}
...
```{stan output.var="compat_gp",eval=FALSE}
data {
  int<lower=1> N;
  real x[N];
}
...
## 贝叶斯广义线性模型  {#sec:bayesian-data-analysis}
