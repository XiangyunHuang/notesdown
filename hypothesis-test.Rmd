...
Jacob Cohen 的著作《Statistical Power Analysis for the Behavioral Sciences》第二版 [@Cohen1988]
...
1. `sig.level` 显著性水平，默认是 0.05 （犯第 I 类错误的概率）

1. `power` 检验的功效（1 - 犯第 II 类错误的概率）

1. `type` t 检验的类型，取值为 `"two.sample"`、 `"one.sample"`或 `"paired"`

1. `alternative` 单边或双边检验，取值为 `"two.sided"` 或 `"one.sided"`
...
R 语言实战 [@Action_2015_Robert] 作者 Robert I. Kabacoff 创建了网站 [Quick-R](https://www.statmethods.net/)，实战这本书第 10 章功效分析主要基于 pwr 包来介绍 -->
