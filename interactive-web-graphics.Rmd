...
```{r plotly-carpet,eval=knitr::is_html_output(),warning=FALSE,fig.cap="scattercarpet 地毯图"}
plot_ly(
  type = "carpet", mode = "markers+lines",
...
```{r plotly-ternary,eval=knitr::is_html_output(),warning=FALSE,fig.cap="三元图 Ternary"}
...
dat <- data.frame(
  ...
  transform(qv_ctr = 100*valid_click_qv / search_qv)
```

例子

```{r multiple-axes,fig.cap="双轴图",eval=knitr::is_html_output()}
# visible = "legendonly" 表示此条曲线隐藏，只在图例里显示，有时候很多条线，默认只是展示几条而已
# hoverinfo = "text" 表示 tooltips 使用指定的 text 
plot_ly(data = dat) %>%
  add_lines(
    x = ~dt, y = ~search_qv, name = "搜索 QV",
    text = ~ paste("搜索 QV：", format(search_qv, big.mark = ",")), hoverinfo = "text",
    line = list(shape = "spline", color = "Set1", width = 1.5, dash = "line")
  ) %>%
  add_lines(
    x = ~dt, y = ~valid_click_qv, name = "有效点击 QV", visible = "legendonly",
    text = ~ paste("有效点击 QV：", format(valid_click_qv, big.mark = ",")), hoverinfo = "text",
    line = list(shape = "spline", color = "Set1", width = 1.5, dash = "line")
  ) %>%
  add_lines(
    x = ~dt, y = ~qv_ctr, name = "QV_CTR", yaxis = "y2", 
    text = ~ paste("点击率：", round(qv_ctr, 2), "%"), hoverinfo = "text",
    line = list(shape = "spline", color = "Set1", width = 1.5, dash = "line")
  ) %>%
  layout(
    title = "",
    ...
    yaxis = list(title = "搜索 QV / 有效点击 QV", showgrid = F, showline = F),
    margin = list(r = 20, autoexpand = T),
    legend = list(
      x = 1.1, y = 1, orientation = "v",
      title = list(text = "<b> 指标 </b>")
    ), bargap = 0.2
  ) %>%
...
### 甘特图 {#subsec:plotly-gantt-charts}
    ...
    annotations = list(
      list(
        ...
        text = paste0(
          ...
          "资源类型: ", length(unique(df$desource)), " 个<br>"
        ),
        font = list(size = 12),
        ax = 0, ay = 0,
        align = "left"
...
### 帕雷托图 {#subsec:plotly-pareto-charts}
...
### 漏斗图 {#subsec:plotly-funnel}

```{r funnel, eval=knitr::is_html_output(), fig.cap="漏斗图"}
dat <- data.frame(
...
### 调色板 {#subsec:plotly-color-palette}
...
```{r plotly-colorpalette,fig.cap="调色板",eval=knitr::is_html_output(),warning=FALSE}
dat <- data.frame(
  dt = rep(seq(
