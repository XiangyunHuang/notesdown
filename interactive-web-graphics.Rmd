...
```{r plotly-ternary,eval=knitr::is_html_output(),fig.cap="三元图 Ternary"}
df <- data.frame(
...
plot_ly(data = df) %>%
  ...
  layout(
    ...
    ternary = list(
      sum = 100,
      aaxis = axis(title = "Journalist"),
      baxis = axis(title = "Developer"),
      caxis = axis(title = "Designer")
    )
  )
```
...
### 时间线 {#subsec:plotly-vistime}


```{r vistime,fig.cap="时间线图"}
library(vistime)
