...
```{r datatable-ggplot2,eval=FALSE}
library(magrittr)
library(ggplot2)
iris %>%
  as.data.table(.) %>%
...
:::
::: {.column width="47.5%" data-latex="{0.475\textwidth}"}
```{r datatable-ggplot2-plot, ref.label="datatable-ggplot2", fig.cap="data.table 和 ggplot2 的结合"}
...
exp.sim <- merge(experiment, simul)
```
...
```{r}
iris[which(sapply(iris, is.numeric))]
```
