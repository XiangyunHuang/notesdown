...
# 置信区间和信仰区间 Confidence or Fiducial Limits
...
dat <- replicate(10^6, expr = {
  x = sample(0:1, size = 10, replace = TRUE, prob = c(0.8, 0.2))
  sum(x)/10
})

# 成功概率 p = 0.2 每个样本量 10
dat <- rbinom(n = 10^4, size = 10, prob = 0.2)/10
table(dat)
# 分布图 y 轴是密度
...
ggplot(data = dat,aes(x = factor(dat), y = Freq, fill = factor(dat))) +
