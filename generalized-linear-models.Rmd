...
简单线性模型（Linear Models，简称 LM），`stats::lm()` 函数可以拟合线性模型，而一般线性模型（General Linear Models，简称 GLM）允许线性模型方差非齐性、存在相关关系，甚至可以扩展到线性混合效应模型，将线性回归模型，方差分析模型，协方差分析模型统一地看待，一般要采用广义最小二乘（Generalized Least Squares，简称 GLS）拟合， `nlme::gls()` 函数实现广义最小二乘拟合线性模型，类似地，`nlme::gnls()` 函数实现广义最小二乘拟合非线性模型。
...
- 大数据集`biglm::bigglm` [speedglm](https://cran.r-project.org/package=speedglm) [bigReg](https://CRAN.R-project.org/package=bigReg)
- `fastglm::fastglm` 加快 `glm` 收敛效果比 `glm` 和 `glm2` 好
- `glm2::glm2` 补充 `glm`
- 有序输出的回归 [lcpm](https://CRAN.R-project.org/package=lcpm) [oglmx](https://CRAN.R-project.org/package=oglmx)
- 多项逻辑回归 gmnl [mlogit](https://CRAN.R-project.org/package=mlogit) [@Train_2009_mlogit]和  mnlogit [@mnlogit_2016_JSS]
- 计数数据回归 pscl
- 条件 GLM [cglm](https://CRAN.R-project.org/package=cglm)
- 多重 GLM [MGLM](https://CRAN.R-project.org/package=MGLM) 多个响应变量
- 有限内存的 GLM [biglmm](https://CRAN.R-project.org/package=biglmm)
- 融合混合分布的稳健 GLM [robmixglm](https://CRAN.R-project.org/package=robmixglm) 
- 聚类数据的 GLM [ClusterBootstrap](https://CRAN.R-project.org/package=ClusterBootstrap) [glmmML](https://CRAN.R-project.org/package=glmmML)
- 广义线性模型 [glmnet](https://CRAN.R-project.org/package=glmnet) <https://glmnet.stanford.edu/>
- [glmpath](https://CRAN.R-project.org/package=glmpath)
