...
TOKEN 保存 GitHub Package [^github-package] 读写删的权限，创建新的 TOKEN，命名为 `GITHUB_PKG` <https://github.com/settings/tokens>
...
# 容器内的信息
docker run --rm -u root -v "/${PWD}://home/docker/" --name=masr-book docker.pkg.github.com/xiangyunhuang/masr/masr-book:devel bash -c "locale; xetex --version; fc-list | sort; python3 -V; pandoc -v; odbcinst -j; Rscript -e 'sessionInfo(.packages(T)); extSoftVersion(); capabilities(); .libPaths(); version; odbc::odbcListDrivers()'; echo '\documentclass{article}\begin{document}hello\end{document}' > test.tex; xelatex test.tex; xelatex test.tex"
```
...
```bash
# 从镜像创建并进入容器，退出容器后销毁容器
docker run -it --rm docker.pkg.github.com/xiangyunhuang/masr/masr-book:devel bash

# 从镜像创建一个叫 masr-book 的容器，并进入该容器，退出容器后不销毁
docker run -it --name=masr-book docker.pkg.github.com/xiangyunhuang/masr/masr-book:devel bash

# 启动容器 masr-book
docker start masr-book
